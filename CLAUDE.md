...
- Old versions kept cards only in each browser (`localStorage` key `vet-translate-cards`); the site offers to merge those into the server once and keeps a `-backup` copy. Merging skips a card whose text already exists anywhere or whose title (first line) already exists in that category (`removeDuplicates` in the Worker; the one-time cleanup of Sep 2026 kept the pre-cleanup state in Durable Object key `backupBeforeDedupeV1`).
- Layout: phone as designed (max 460px); ≥700px one wider column; ≥1000px translator left (sticky) + cards right; ≥1200px cards in two columns. Extra languages and mic language stay per-device.
